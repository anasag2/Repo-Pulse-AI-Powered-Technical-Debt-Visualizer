...
export default function History() {
  ...
  // building's "View history" opens straight to that file's trends.
  const initial = new URLSearchParams(window.location.search);
  const deepLinkRepo = initial.get("repo");
  const deepLinkPath = initial.get("path");

  // Otherwise fall back to whatever was open last time, so leaving and
  // returning to this page (with no deep link) resumes it. The deep link
  // always wins over a stale persisted selection, from the very first render.
  const [picked, setPicked] = usePersistedState<number | null>(
    "history-picked",
    null,
    deepLinkRepo ? parseInt(deepLinkRepo, 10) : undefined,
  );
  ...
  const [codeOnly, setCodeOnly] = usePersistedState("history-codeOnly", true);
  const [fileSearch, setFileSearch] = usePersistedState("history-fileSearch", "");
  const repo = list.find((r) => r.id === picked) ?? null;
